    ...
    sortOptions.forEach((option) => {
        it(`should correctly sort products based on "${option}"`, async () => {
            ...
            if (option.includes('Price')) {
                ...
                const priceValues = await Promise.all(priceElements.map(el => el.getText().then(text => parseFloat(text.replace('$', '').trim()))));
                
                const sortedPrices = [...priceValues].sort((a, b) => option.includes('low to high') ? a - b : b - a);
                expect(priceValues).toEqual(sortedPrices);
            } 
            else {
                const productNameElements = await $$('[data-test="inventory-item-name"]');
                const productNames = await Promise.all(productNameElements.map(el => el.getText()));
